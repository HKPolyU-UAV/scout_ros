...
Vec2 Poistion_controller_PID(Vec3 pose_XYyaw, Vec2 setpoint){ // From VRPN XY position
    ...
    cout << "output_raw v: " << output[0] << " av: " << output[1] << endl;
      if(output[0] >  0.8){ output[0]= 0.8;} //Clamp the forward speed to 1.5 m/s

      if(output[1] >  1.5){ output[1]= 1.5;}
      if(output[1] < -1.5){ output[1]= 1.5;}
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber ugvdespose_sub = nh.subscribe<geometry_msgs::PoseStamped>("/scout_wp/pose", 1, UGVdesPose_cb);
